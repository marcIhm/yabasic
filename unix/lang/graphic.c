...
static void transform(double *,
                      double *); /* do transformation of coordinates */
static void rtransform(double *, double *); /* reverse transformation */
static void itransform(int *, int *);       /* integer variant of transform() */
static void irtransform(int *, int *); /* integer variant of rtransform() */
static int grafinit(void);     /* initialize grafics (either X or Windows) */
static char *newrgb(int, int); /* create a new bitmap string */
static void addrgb(char *, unsigned short, unsigned short,
                   unsigned short); /* add one rgb pixel to bitstring */
static int
readrgb(char *, int, unsigned short *, unsigned short *,
        unsigned short *); /* read rgb pixel from string one after another */
static int change_font(char *); /* change font */
static int rgb_to_pixel(int, int,
                        int); /* create a pixel value from rgb values */
...
static int deleteprinterfile = FALSE; /* true, if we created the filename
                                         ourself an can delete it on close */
...
int mousex = 0, mousey = 0, mouseb = 0,
    mousemod = 0;         /* last known mouse coordinates */
...
XVisualInfo
    visualinfo; /* visual info; contains masks for red, green and blue */
static unsigned long rbits_shift =
    0; /* position of red bits within pixel value */
...
void openwin(struct command *cmd) {
  /* open a Window */
  int fnt;
  ...
  window = XCreateWindow(
      display, root, winx, winy, winwidth, winheight, 0, CopyFromParent,
      CopyFromParent, visualinfo.visual,
      CWBackingStore | CWSaveUnder | CWBackPixel | CWColormap, &attrib);
  ...
  if (backpid == 0) {
	...
	if (severity_threshold <= sNOTE && x11_note_on_receive_expose) {
	  error(sNOTE, "Received X11-event Expose");
        ...
        patch = XGetImage(display, backbit, rx, ry, rw, rh, vals.plane_mask,
                          ZPixmap);
        if (patch) {
          XPutImage(display, window, gc, patch, 0, 0, rx, ry, rw, rh);
          XDestroyImage(patch);
        }
      }
    }
  ...
  wthandle = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)winthread, 0, 0,
                          (LPDWORD)&wtid);
  if (wthandle == NULL) {
...
#ifdef WINDOWS
LRESULT CALLBACK mywindowproc(HWND handle, unsigned msg, UINT wparam,
                              DWORD lparam) {
  ...
  case WM_PAINT:
    if (GetUpdateRect(handle, &cr, 0)) {
      ...
      BitBlt(ps.hdc, ps.rcPaint.left, ps.rcPaint.top,
             ...
             ps.rcPaint.bottom - ps.rcPaint.top, bitcon, ps.rcPaint.left,
...
static DWORD winthread(LPWORD par) {
  ...
  AdjustWindowRect(&cr, WS_VISIBLE | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX,
                   FALSE);
  ...
  cr.bottom = winheight;
  backbit =
      CreateCompatibleBitmap(devcon, cr.right - cr.left, cr.bottom - cr.top);
...
static void startdraw(int clear) {
  /* prepare for drawing */
  RECT interior;
  ...
  IntersectClipRect(devcon, interior.left, interior.top, interior.right,
                    interior.bottom);
  if (printer) {
    if (clear) {
...
static int grafinit(void) {
  ...
  if (!XMatchVisualInfo(display, DefaultScreen(display), 24, TrueColor,
      ...
      !XMatchVisualInfo(display, DefaultScreen(display), 32, TrueColor,
                        &visualinfo) &&
      !XMatchVisualInfo(display, DefaultScreen(display), 16, TrueColor,
                        &visualinfo) &&
      !XMatchVisualInfo(display, DefaultScreen(display), 12, TrueColor,
                        &visualinfo) &&
  ...
  /* now, that we have the colormap, we try to get the colors requested for
   * fore- and background */
  if (!foreground) {
  ...
  if (!XLookupColor(display, colormap, foreground, &exact_match, &best_match)) {
    sprintf(string, "Could not find foreground color '%s'\n", background);
    error(sERROR, string);
    return FALSE;
  }
  forepixel = rgb_to_pixel(best_match.red >> 8, best_match.green >> 8,
                           best_match.blue >> 8);
  ...
  if (!XLookupColor(display, colormap, background, &exact_match, &best_match)) {
    sprintf(string, "Could not find background color '%s'\n", background);
    error(sERROR, string);
    return FALSE;
  }
  backpixel = rgb_to_pixel(best_match.red >> 8, best_match.green >> 8,
                           best_match.blue >> 8);
  ...
  if (sscanf(foreground, "%d,%d,%d", &r, &g, &b) != 3 || r < 0 || r > 255 ||
      g < 0 || g > 255 || b < 0 || b > 255) {
    sprintf(string,
            "command line option -foreground must be three numbers between 0 "
            "and 255, separated by commas (not '%s')",
            foreground);
    error(sERROR, string);
    return FALSE;
  }
  ...
  if (sscanf(background, "%d,%d,%d", &r, &g, &b) != 3 || r < 0 || r > 255 ||
      g < 0 || g > 255 || b < 0 || b > 255) {
    sprintf(string,
            "command line option -background must be three numbers between 0 "
            "and 255, separated by commas (not '%s')",
            background);
    error(sERROR, string);
    return FALSE;
...
static int change_font(char *fname) {
  /* change font */
#ifdef WINDOWS
  int n;
...
void dot(struct command *cmd) {
  ...
  if (printerfile) {
    fprintf(printerfile, "%g %g (%c) DO\n", (x - 0.5) * psscale,
            (winheight - y + 0.5) * psscale, clear ? 'y' : 'n');
  }
#elif WINDOWS
  startdraw(clear);
  if (clear) {
...
void create_line(int numpoints) {
  /* create Command 'line' */
  struct command *cmd;
...
void line(struct command *cmd) {
  ...
  if (printerfile) {
    fprintf(printerfile, "%g %g %g %g (%c) LI\n", x1 * psscale,
            (winheight - y1) * psscale, x2 * psscale,
            (winheight - y2) * psscale, clear ? 'y' : 'n');
  }
  fflush(printerfile);
#elif WINDOWS
  startdraw(clear);
...
void change_colour(struct command *cmd) {
  ...
  if (cmd->type == cGCOLOUR || cmd->type == cGBACKCOLOUR) {
    ...
    if (r < 0 || r > 255 || g < 0 || g > 255 || b < 0 || b > 255) {
      sprintf(string,
              "arguments to command colour must be between 0 and 255 (not "
              "%d,%d,%d)",
              r, g, b);
      error(sERROR, string);
      return;
    ...
    if (sscanf(h, "%d,%d,%d", &r, &g, &b) != 3 || r < 0 || r > 255 || g < 0 ||
        g > 255 || b < 0 || b > 255) {
      sprintf(string,
              "string argument to command colour must be three numbers between "
              "0 and 255, separated by commas (not '%s')",
              h);
      error(sERROR, string);
      return;
    }
...
int rgb_to_pixel(int r, int g, int b) {
  /* create a pixel value from rgb values */
  long pixel;

#ifdef UNIX
  pixel =
      (visualinfo.red_mask & ((long)(r * rbits_max / 255) << rbits_shift)) +
      (visualinfo.green_mask & ((long)(g * gbits_max / 255) << gbits_shift)) +
      (visualinfo.blue_mask & ((long)(b * bbits_max / 255) << bbits_shift));
...
static void pixel_to_rgb(int *r, int *g, int *b, int pixel) {
...
void circle(struct command *cmd) {
  /* draw a circle */
  double x, y, r;
  int fill, clear;
  ...
  if (clear) {
    if (fill) {
      XFillArc(display, window, rgc, x - r, y - r, 2 * r, 2 * r, 0 * 64,
               360 * 64);
      XFillArc(display, backbit, rgc, x - r, y - r, 2 * r, 2 * r, 0 * 64,
               360 * 64);
    } else {
      XDrawArc(display, window, rgc, x - r, y - r, 2 * r, 2 * r, 0 * 64,
               360 * 64);
      XDrawArc(display, backbit, rgc, x - r, y - r, 2 * r, 2 * r, 0 * 64,
               360 * 64);
    }
  } else {
    if (fill) {
      XFillArc(display, window, gc, x - r, y - r, 2 * r, 2 * r, 0 * 64,
               360 * 64);
      XFillArc(display, backbit, gc, x - r, y - r, 2 * r, 2 * r, 0 * 64,
               360 * 64);
    } else {
      XDrawArc(display, window, gc, x - r, y - r, 2 * r, 2 * r, 0 * 64,
               360 * 64);
      XDrawArc(display, backbit, gc, x - r, y - r, 2 * r, 2 * r, 0 * 64,
               360 * 64);
    }
  }
  ...
  if (printerfile) {
    ...
    fprintf(printerfile, "%g %g %g (%c) (%c) CI\n", (winheight - y) * psscale,
            x * psscale, r * psscale, clear ? 'y' : 'n', fill ? 'y' : 'n');
    fflush(printerfile);
  }
#else /* WINDOWS */
  startdraw(clear);
  if (fill) {
    Ellipse(devcon, (int)(x - r), (int)(y - r), (int)(x + r), (int)(y + r));
    Ellipse(bitcon, (int)(x - r), (int)(y - r), (int)(x + r), (int)(y + r));
    if (printer) {
    ...
    Arc(devcon, (int)(x - r), (int)(y - r), (int)(x + r), (int)(y + r), 0, 0, 0,
        0);
    Arc(bitcon, (int)(x - r), (int)(y - r), (int)(x + r), (int)(y + r), 0, 0, 0,
        0);
    if (printer) {
...
void triangle(struct command *cmd) {
  ...
  if (clear) {
    if (fill) {
      XFillPolygon(display, window, rgc, points, 4, Convex, CoordModeOrigin);
      XFillPolygon(display, backbit, rgc, points, 4, Convex, CoordModeOrigin);
    } else {
    ...
    if (fill) {
      ...
      XFillPolygon(display, backbit, gc, points, 4, Convex, CoordModeOrigin);
    } else {
  ...
  if (printerfile) {
    ...
    fprintf(printerfile, "%g %g %g %g %g %g (%c) (%c) TRI\n", x0 * psscale,
            (winheight - y0) * psscale, x1 * psscale,
            (winheight - y1) * psscale, x2 * psscale,
            (winheight - y2) * psscale, clear ? 'y' : 'n', fill ? 'y' : 'n');
    fflush(printerfile);
  }
#else /* WINDOWS */
  startdraw(clear);
...
void text(struct command *cmd) {
  ...
  if (arg2) {
    if (check_alignment(arg2)) {
      if (align) {
        sprintf(string, "Found two possible alignments: '%s' and '%s'", arg1,
                arg2);
        error(sERROR, string);
      }
      align = arg2;
  ...
  if (printerfile) {
    ...
    if (firsttext) {
      firsttext = FALSE;
      XTextExtents(myfont, sample, strlen(sample), &d1, &d2, &d3, &size);
      fprintf(printerfile, "/Helvetica findfont setfont newpath 0 0 moveto\n");
      ...
      fprintf(printerfile, "/Helvetica findfont exch scalefont setfont\n");
    }
    fprintf(printerfile, "%g %g (%c) (%s) AT\n", x * psscale,
            (winheight - y - yoff) * psscale, align[0], string);
  }
#else /* WINDOWS */
  startdraw(FALSE);
  ...
  if (printer) {
    ...
    TextOut(printer, (int)((x + xoff) * prnscale + prnoffx),
            (int)((y + yoff) * prnscale + prnoffy), text, strlen(text));
  }
#endif
}

int check_alignment(char *al) {
...
static void itransform(int *ix, int *iy) {
...
static void irtransform(int *ix, int *iy) {
  /* integer variant of rtransform() */
  double dx, dy;
...
static void transform(double *x, double *y) {
  /* do coordinate transformation from coordinates of yabasic
     (regarding origin) to coordinates of graphics-system */
  double xz, yz, xd, yd;
  double xold, yold;
  ...
  *y = yz + (*y) * yd;
  ...
  if (severity_threshold <= sDEBUG) {
    sprintf(string, "transforming (%g,%g) into (%g,%g)", xold, yold, *x, *y);
    error(sDEBUG, string);
  }
}

static void rtransform(double *x, double *y) {
  /* do reverse coordinate transformation */
  double xz, yz, xd, yd;
  double xold, yold;
...
void rect(struct command *cmd) {
  ...
  if (clear) {
    if (fill) {
      XFillRectangle(display, window, rgc, x1, y1, x2 - x1 + 1, y2 - y1 + 1);
      XFillRectangle(display, backbit, rgc, x1, y1, x2 - x1 + 1, y2 - y1 + 1);
    } else {
    ...
    if (fill) {
      ...
      XFillRectangle(display, backbit, gc, x1, y1, x2 - x1 + 1, y2 - y1 + 1);
    } else {
...
void putbit(void) {
  ...
  if (!strncmp(mode, "solid", strlen(mode))) {
    ...
    sprintf(string,
            "Invalid mode for putbit: '%s', only 'solid' and 'transparent' are "
            "allowed",
            mode);
    error(sERROR, string);
    return;
  ...
  if (xe + we > 0 && ye + he > 0) {
    XGetGCValues(display, gc, GCPlaneMask, &vals);
    bits =
        XGetImage(display, backbit, xe, ye, we, he, vals.plane_mask, XYPixmap);
    if (!bits) {
  ...
  for (y = 0; y < h; y++) {
    for (x = 0; x < w; x++) {
      if (!readrgb(NULL, 0, &red, &green, &blue)) {
        sprintf(estring,
                "error while reading line %d of %d, pixel %d of %d from string",
                y + 1, h, x + 1, w);
...
#ifdef UNIX
          XPutPixel(bits, x - xe + xdest, y - ye + ydest, should_pixel);
#else
...
char *getbit(int x1, int y1, int x2, int y2) {
  /* get rect from win */
  int s, x, y, pixel;
  int xe1, ye1, xe2, ye2;
  char *bitstring;
  ...
  if (xe1 <= xe2 && ye1 <= ye2) {
    XGetGCValues(display, gc, GCPlaneMask, &vals);
    bits = XGetImage(display, backbit, xe1, ye1, xe2 - xe1 + 1, ye2 - ye1 + 1,
                     vals.plane_mask, XYPixmap);
    if (!bits) {
      error(sERROR, "Couldn't get bits from window");
      return my_strdup("");
    }
  ...
  for (y = y1; y <= y2; y++) {
    for (x = x1; x <= x2; x++) {
      ...
      addrgb(bitstring, GetRValue(pixel), GetGValue(pixel), GetBValue(pixel));
    }
  }
  enddraw();
#endif
...
static char *newrgb(int w, int h) {
...
static void addrgb(char *bits, unsigned short red, unsigned short green,
                   unsigned short blue) {
...
static int readrgb(char *bits, int off, unsigned short *red,
  ...
  if (scanned == EOF)
    ...
    sprintf(estring,
            "Invalid bitmap: could only extract %d hex-values from supplied "
            "string at position %ld: '%.6s'",
            scanned, bitpt - bitstart, bitpt);
  error(sERROR, estring);
  return 0;
}

void create_openprinter(int num) {
...
void openprinter(struct command *cmd) {
  /* opens a printer */
#ifdef WINDOWS
  ...
  if (*printerfilename == '\0') {
    ...
    printerfilename = my_strdup("/tmp/yabasic-postscript-output-XXXXXX");
    ...
    if (stat(printerfilename, &s) && errno != ENOENT) {
      sprintf(string, "could not check printerfile '%s': %s", printerfilename,
              my_strerror(errno));
      error(sERROR, string);
      return;
    }
    if (s.st_mode & S_IFLNK) {
      sprintf(string, "could not print to file '%s'; it is a symbolic link");
      error(sERROR, string);
      return;
    }
  ...
  if (first) {
    ...
    *p = '\0';
    ...
    IntersectClipRect(printer, (int)(interior.left * prnscalex + prnoffx),
  ...
  fprintf(printerfile, "/CLYN {(y) eq {1 setgray} {rgb /r get rgb /g get rgb "
                       "/b get setrgbcolor} ifelse} def\n");
  fprintf(printerfile,
...
void getwinkey(char *retkey) {
  ...
  do {
    ...
    len = XLookupString((XKeyPressedEvent *)&event, retkey, 100, &sym, NULL);
    if (event.type == KeyPress) {
      ...
      if (keysym) {
        ...
        keysym = NULL;
           ...
           (sym == XK_Shift_L || sym == XK_Shift_R || sym == XK_Control_L ||
