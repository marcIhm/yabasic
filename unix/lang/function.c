...
struct command *lastdata =
    NULL; /* used to associate all data-commands with each others */
...
void tokenalt(struct command *cmd) /* extract token from variable with alternate
                                      semantics */
{
...
void function(struct command *cmd) /* performs a function */
{
  ...
  case fSTR3:
    result = stSTRING;
    if (!myformat(string, INBUFFLEN, a1->value, a2->pointer,
                  a3 ? a3->pointer : NULL)) {
      pointer = my_strdup("");
      break;
    }
  ...
  case fGETBIT:
    pointer =
        getbit((int)a1->value, (int)a2->value, (int)a3->value, (int)a4->value);
    result = stSTRING;
    break;
  case fGETCHAR:
    pointer = getchars((int)a1->value, (int)a2->value, (int)a3->value,
                       (int)a4->value);
    result = stSTRING;
    break;
  case fTELL:
...
static int do_system(
    char *cmd) /* hand over execution of command to system, return exit code */
{
#ifdef UNIX
  int ret;
  if (con_xcap_inized) {
  ...
  if (!CreateProcess(NULL, string, &prosec, &thrsec, TRUE, 0, NULL, NULL,
                     &start, &proc)) {
...
static char *do_system2(char *cmd) /* hand over execution of command to system,
                                      return output as string */
{
...
void getmousexybm(char *s, int *px, int *py, int *pb,
                  int *pm) /* get mouse coordinates */
{
  int x = 0, y = 0, b = 0, m = 0;
  char c;
...
static char *dec2other(double d,
                       int base) /* convert double to hex or binary number */
{
  int len;
  double dec, dec2;
  char *other;
  ...
  for (len--; len >= 0; len--) {
    other[len] =
        "0123456789abcdef"[(int)(floor(dec - base * floor(dec / base) + 0.5))];
    dec = floor(dec / base);
  }
  if (negative) {
    other[0] = '-';
...
static double other2dec(char *hex,
                        int base) /* convert hex or binary to double number */
{
  double dec;
...
int myformat(char *dest, int max, double num, char *format,
             char *sep) /* format number according to string */
{
  int ret = myformat2(dest, max, num, format, sep);
  if (ret == 0)
    return TRUE;
  if (ret == 1)
    sprintf(estring, "'%s' is not a valid format", format);
  if (ret == 2)
    sprintf(estring, "length of formatted string exceeds maximum of %d bytes",
            INBUFFLEN);
  error(sERROR, estring);
  return FALSE;
}

int myformat2(char *dest, int max, double num, char *format,
              char *sep) /* do the work for myformat */
{
  ...
  if (*form == '%') {
    ...
    /* disassemble in integer and fractional part; both ip and fp will be
     * consumed stepwise in the process */
    fp = modf(num, &ipdbl);
    ip = (uint64_t)ipdbl;

    /* variable cr serves as our cursor running from right to left and marks the
     * position to be written next */
    ...
    /* given format does not have enough room, this is an error; just copy
     * format into dest and return */
    if ((neg && cr < 0) || ip) {
      strcpy(dest, format);
      return 0;
    }
    ...
    /* until now we used fixed separators ',' for thousands and '.' for decimal;
       but if user has given his own separators (e.g. german or swiss style) we
...
static char *fromto(char *str, int from,
                    int to) /* gives back portion of string */
...
void poke(struct command *cmd) /* poke into internals */
{
  ...
  if (!strcmp(dest, "fontheight") && !string_arg) {
    ...
    if (count != (int)double_arg) {
      sprintf(string,
              "assertion failed for number of entries on stack; expected = %d, "
              "actual = %d",
              (int)double_arg, count);
      error(sFATAL, string);
    }
  } else if (dest[0] == '#') {
...
static double peek(char *dest) /* peek into internals */
{
  ...
  if (!strcmp(dest, "winwidth")) {
    ...
    return is_bound;
  } else if (!strcmp(dest, "last_foreign_function_call_okay") ||
             !strcmp(dest, "last_frnfn_call_okay")) {
...
static int peekfile(int s) /* read a byte from stream */
{
  if (s && badstream(s, 0)) {
    return 0;
  }
...
static char *peek2(char *dest, struct command *curr) /* peek into internals */
{
  ...
  } else if (!strcmp(dest, "last_foreign_function_call_error_text") ||
             !strcmp(dest, "last_frnfn_call_error_text")) {
...
void restore(struct command *cmd) /* reset data pointer to given label */
{
  ...
  if (cmd->type == cRESTORE) {
    ...
    *datapointer = label;
    if (lastdata) {
      while ((*datapointer)->type != cDATA && (*datapointer) != cmd_head) {
...
void readdata(struct command *cmd) /* read data items */
{
  ...
  type = cmd->tag;
  while (*datapointer &&
         ((*datapointer)->type != cDATA || cmd->lib != (*datapointer)->lib)) {
...
void dblrelop(struct command *cmd) /* compare topmost double-values */
{
  double a, b, c;
...
void strrelop(struct command *cmd) /* compare topmost string-values */
{
  char *a, *b;
  double c;
  struct stackentry *result;
...
void switch_compare(void) /* compare topmost values for switch statement */
{
  ...
  if ((second->type == stSWITCH_STRING || second->type == stSTRING) &&
    ...
    error(sERROR, "mixing strings and numbers in a single switch statement is "
                  "not allowed");
  }
   ...
   */
  if (r == 1.) {
    if (second->type == stNUMBER)
      second->type = stSWITCH_NUMBER;
    if (second->type == stSTRING)
      second->type = stSWITCH_STRING;
  }
...
void logical_shortcut(struct command *cmd) /* shortcut and/or if possible */
{
  struct stackentry *result;
  double is;
...
void boole(struct command *cmd) /* perform and/or/not */
{
  int a, b, c;
  struct stackentry *result;
