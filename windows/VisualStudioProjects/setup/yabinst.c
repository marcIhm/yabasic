...
written by Marc-Oliver Ihm in 1996.
  ...
  Date of last change:
*/
#define DOLC                     "March 21, 2014"
/*

This program is copyrighted, you may distribute it under
the same terms as yabasic, i.e. under the Artistic License or
the GNU General Public License at your choice.
  ...
  if (install) {   /* install yabasic */
    ...
    if (oldversion>newversion) {
      sprintf(string,"A newer Version of "BASIC_NAME
        ...
        "  \talready installed: \t%5.3f\n"
        "  \tto be installed: \t%5.3f\n"
...
int MyRegs(int mode) /* add or delete entries to or from registry */
{
  ...
  case INSTALL:
    ...
    sprintf(string,"%5.3f",newversion);
    progress(NULL);
