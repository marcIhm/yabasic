...
** A typical building sequence
   ...
   - do git :: git pull/add/commit/push appropriately, as all changes between the platforms unix
               and windows are only propagated by git; frequent commit messages are "new version"
               or "build windows". A more meaningful message (taken from the file NEWS) will be
               added when creating a new tag (during do rake) below.
